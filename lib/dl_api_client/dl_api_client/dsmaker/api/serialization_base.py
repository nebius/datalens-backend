...
class BaseApiV1SerializationAdapter:
    ...
    def dump_item(self, item: ApiProxyObject, action: Action = None) -> dict:  # type: ignore  # 2024-01-22 # TODO: Incompatible default for argument "action" (default has type "None", argument has type "Action")  [assignment]
        """
    ...
    def dump_data_source(self, item: DataSource, action: Action) -> dict:
        if action == Action.delete:
            ...
            return dict(
                ...
                source_type=item.source_type.name,  # type: ignore  # 2024-01-22 # TODO: Item "None" of "DataSourceType | None" has no attribute "name"  [union-attr]
                title=item.title,
                raw_schema=[
                    dict(
                        name=col.name,
    ...
    def dump_obligatory_filter(self, item: ObligatoryFilter, action: Action = None) -> dict:  # type: ignore  # 2024-01-22 # TODO: Incompatible default for argument "action" (default has type "None", argument has type "Action")  [assignment]
        if action == Action.delete:
            return dict(id=item.id)
        if action == Action.add:
    ...
    def dump_updates(self, updates: List[Union[UpdateAction, dict]] = None) -> List[dict]:  # type: ignore  # 2024-01-22 # TODO: Incompatible default for argument "updates" (default has type "None", argument has type "list[UpdateAction | dict[Any, Any]]")  [assignment]
        result = []
