...
class SyncHttpDatasetApiV1(SyncHttpApiV1Base):
    ...
    def save_dataset(
        ...
        preview: bool = True,
        fail_ok: bool = False,
        dir_path: str = None,  # type: ignore  # 2024-01-22 # TODO: Incompatible default for argument "dir_path" (default has type "None", argument has type "str")  [assignment]
        workbook_id: str = None,  # type: ignore  # 2024-01-22 # TODO: Incompatible default for argument "workbook_id" (default has type "None", argument has type "str")  [assignment]
        created_via: str = None,  # type: ignore  # 2024-01-22 # TODO: Incompatible default for argument "created_via" (default has type "None", argument has type "str")  [assignment]
        lock_timeout: int = 3,
    ...
    def copy_dataset(
        ...
        assert response.status_code == 200, f"Dataset copy fail: {response.data}"  # type: ignore  # 2024-01-22 # TODO: If x = b'abc' then f"***x***" or "***".format(x) produces "b'abc'", not "abc". If this is desired behavior, use f"***x!r***" or "***!r***".format(x). Otherwise, decode the bytes  [str-bytes-safe]
    ...
    def apply_updates(  # type: ignore  # TODO: fix
        ...
        updates = list(updates or ()) + self.serial_adapter.generate_implicit_updates(dataset)  # type: ignore  # 2024-01-22 # TODO: Unsupported operand types for + ("list[UpdateAction | dict[Any, Any]]" and "list[UpdateAction]")  [operator]
        ...
        if fail_ok:
            ...
            try:
                ...
                print("\n".join(HttpApiResponse.extract_response_errors(response)))  # type: ignore  # 2024-01-22 # TODO: Argument 1 to "extract_response_errors" of "HttpApiResponse" has incompatible type "ClientResponse"; expected "dict[str, Any]"  [arg-type]
                raise
