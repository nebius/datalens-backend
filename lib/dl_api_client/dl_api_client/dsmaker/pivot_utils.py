...
from dl_api_client.dsmaker.data_abstraction.mapping_comparator import SimpleMapper1DComparator
...
from dl_api_client.dsmaker.data_abstraction.result import ResultDataAbstraction
...
def get_pivot_response(
    ...
    def make_pivot_item(
        ...
        if title == MEASURE_NAME_TITLE:
            return dataset.make_req_pivot_item(role=role, legend_item_ids=legend_item_ids, title=remapped_title)
        if role == PivotRole.pivot_annotation:
            return dataset.make_req_pivot_item(
                role=role, legend_item_ids=legend_item_ids, title=remapped_title, annotation_type="color"
            )
        if role in (PivotRole.pivot_row, PivotRole.pivot_column):
            return dataset.make_req_pivot_item(
                ...
                direction=order_fields.get(title, OrderDirection.asc),  # type: ignore  # 2024-01-22 # TODO: Argument "direction" to "make_req_pivot_item" of "Dataset" has incompatible type "OrderDirection | None"; expected "OrderDirection"  [arg-type]
            )
    ...
    pivot_resp = data_api.get_pivot(
        dataset=dataset,
        fields=legend,  # type: ignore  # 2024-01-22 # TODO: Argument "fields" to "get_pivot" of "SyncHttpDataApiV2" has incompatible type "list[RequestLegendItem]"; expected "list[ResultField | RequestLegendItem] | None"  [arg-type]
        pivot_structure=[
        ...
        order_by=[order_by_item(title=title, direction=direction) for title, direction in (order_fields or {}).items()],  # type: ignore  # 2024-01-22 # TODO: Argument "direction" to "order_by_item" has incompatible type "OrderDirection | None"; expected "OrderDirection"  [arg-type]
        filters=filters,
...
def check_pivot_response(
    ...
    if check_totals is None:
        ...
        if simple_totals is not None:
            if not measures:
                ...
                check_totals = (
                    (
                        [tuple(rows[: row_tot_item.level]) + tuple(columns) for row_tot_item in simple_totals.rows]
                        if rows
                        else []
                    )
                    + (
                        [tuple(columns[: col_tot_item.level]) + tuple(rows) for col_tot_item in simple_totals.columns]
                        if columns
                        else []
                    )
                    + (
    ...
    actual_pivot_legend_tuples = [(item.title, item.role_spec.role) for item in pivot_resp.data["pivot"]["structure"]]
    ...
    result_resp = data_api.get_result(dataset=dataset, fields=legend_for_result, filters=filters, fail_ok=True)  # type: ignore  # 2024-01-22 # TODO: Argument "fields" to "get_result" of "SyncHttpDataApiV2" has incompatible type "list[RequestLegendItem]"; expected "list[ResultField | RequestLegendItem] | None"  [arg-type]
    assert result_resp.status_code == HTTPStatus.OK, result_resp.json

    result_mapper_measure_liids = frozenset(
        chain.from_iterable(liid_map[title] for title in chain(measures, annotations or ()))
    )
    ...
    def _is_total(dimension: Optional[list], totals_liids: set[int]):  # type: ignore  # 2024-01-22 # TODO: Function is missing a return type annotation  [no-untyped-def]
        return dimension is not None and any(value[0][1] in totals_liids for value in dimension)

    totals_liids = {
        item.legend_item_id for item in pivot_resp.data["fields"] if item.role_spec.role.value == "template"
    }
    ...
    if expected_notifications is not None:
        actual_notifications = [
            NotificationType(notification["locator"]) for notification in pivot_resp.data["notifications"]
        ]
    ...
    expected_row_dim_headers = [
        item.title for item in pivot_resp.data["pivot"]["structure"] if item.role_spec.role == PivotRole.pivot_row
    ]
...
def get_all_measure_cells(pivot_rows: list[dict]) -> list[list[list]]:
    return [cell for row in pivot_rows for cell in row["values"] if cell is not None]  # Each cell is a list of pairs
