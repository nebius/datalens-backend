...
class ApiProxyObject:
    ...
    def update(self, **data) -> UpdateAction:  # type: ignore  # 2024-01-22 # TODO: Function is missing a type annotation for one or more arguments  [no-untyped-def]
...
class Container(Generic[_ITEM_TV]):
    ...
    def __init__(self, data: Union[list, tuple, dict, "Container"] = None):  # type: ignore  # 2024-01-22 # TODO: Incompatible default for argument "data" (default has type "None", argument has type "list[Any] | tuple[Any, ...] | dict[Any, Any] | Container[Any]")  [assignment]
...
class DataSource(ApiProxyObject):
    ...
    source_type: Optional[DataSourceType] = attr.ib(default=None, converter=DataSourceType.normalize)  # type: ignore  # 2024-01-22 # TODO: Unsupported converter, only named functions, types and lambdas are currently supported  [misc]
...
class SourceAvatar(ApiProxyObject):
    ...
    def join(
        self,
        other: "SourceAvatar",
        conditions: List["JoinCondition"] = None,  # type: ignore  # 2024-01-22 # TODO: Incompatible default for argument "conditions" (default has type "None", argument has type "list[JoinCondition]")  [assignment]
...
class _Column:
    ...
    user_type: Optional[UserDataType] = attr.ib(default=None, converter=UserDataType.normalize)  # type: ignore  # 2024-01-22 # TODO: Unsupported converter, only named functions, types and lambdas are currently supported  [misc]
...
def _make_pivot_role_spec(
    ...
    if role == PivotRole.pivot_annotation:
        pivot_role_spec = AnnotationPivotRoleSpec(
            role=role,
            annotation_type=annotation_type,  # type: ignore  # 2024-01-22 # TODO: Argument "annotation_type" to "AnnotationPivotRoleSpec" has incompatible type "str | None"; expected "str"  [arg-type]
...
class _ResultField(ApiProxyObject):
    ...
    calc_mode: CalcMode = attr.ib(default=CalcMode.direct, converter=CalcMode.normalize)  # type: ignore  # 2024-01-22 # TODO: Unsupported converter, only named functions, types and lambdas are currently supported  [misc]
    aggregation: AggregationFunction = attr.ib(
        default=AggregationFunction.none, converter=AggregationFunction.normalize  # type: ignore  # 2024-01-22 # TODO: Unsupported converter, only named functions, types and lambdas are currently supported  [misc]
    )
    type: FieldType = attr.ib(default=FieldType.DIMENSION, converter=FieldType.normalize)  # type: ignore  # 2024-01-22 # TODO: Unsupported converter, only named functions, types and lambdas are currently supported  [misc]
    source: Optional[str] = attr.ib(default=None)
    hidden: bool = attr.ib(default=False)
    description: str = attr.ib(default="")
    formula: str = attr.ib(default="")
    initial_data_type: Optional[UserDataType] = attr.ib(default=None, converter=UserDataType.normalize)  # type: ignore  # 2024-01-22 # TODO: Unsupported converter, only named functions, types and lambdas are currently supported  [misc]
    cast: Optional[UserDataType] = attr.ib(default=None, converter=UserDataType.normalize)  # type: ignore  # 2024-01-22 # TODO: Unsupported converter, only named functions, types and lambdas are currently supported  [misc]
    data_type: Optional[UserDataType] = attr.ib(default=None, converter=UserDataType.normalize)  # type: ignore  # 2024-01-22 # TODO: Unsupported converter, only named functions, types and lambdas are currently supported  [misc]
    valid: bool = attr.ib(default=True)
    has_auto_aggregation: bool = attr.ib(default=False)
    lock_aggregation: bool = attr.ib(default=False)
    avatar_id: Optional[str] = attr.ib(default=None)
    managed_by: ManagedBy = attr.ib(default=ManagedBy.user, converter=ManagedBy.normalize)  # type: ignore  # 2024-01-22 # TODO: Unsupported converter, only named functions, types and lambdas are currently supported  [misc]
    ...
    def parameter_value(self, value: Any = None) -> ParameterFieldValue:
        return ParameterFieldValue(field_id=self.id, value=value if value is not None else self.default_value.value)  # type: ignore  # 2024-01-22 # TODO: Item "None" of "ParameterValue[Any] | None" has no attribute "value"  [union-attr]
...
class Dataset(ApiProxyObject):
    ...
    def measure_name_as_req_legend_item(
        ...
        if role == FieldRole.row:
            ...
            role_spec = RoleSpec(role=role)  # type: ignore  # 2024-01-22 # TODO: Incompatible types in assignment (expression has type "RoleSpec", variable has type "RowRoleSpec")  [assignment]
        return RequestLegendItem(
