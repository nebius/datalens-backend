...
class CryptoKeysConfig(SettingsBase):
    map_id_key: Dict[str, str] = s_attrib("KEY_VAL_ID", sensitive=True)  # type: ignore  # 2024-01-22 # TODO: Incompatible types in assignment (expression has type "Attribute[Any]", variable has type "dict[str, str]")  [assignment]
    actual_key_id: str = s_attrib("ACTUAL_KEY_ID")  # type: ignore  # 2024-01-22 # TODO: Incompatible types in assignment (expression has type "Attribute[Any]", variable has type "str")  [assignment]

    @actual_key_id.default  # type: ignore  # 2024-01-22 # TODO: "str" has no attribute "default"  [attr-defined]
    def _default_actual_key_id(self):  # type: ignore  # 2024-01-22 # TODO: Function is missing a return type annotation  [no-untyped-def]
    ...
    def from_json(cls, json_value: Dict[str, Any]) -> CryptoKeysConfig:
        ...
        try:
            ...
            # TODO FIX: Integrate with env loader exceptions handling
            raise ValueError(f"Crypto keys config missing key {exc!r}")
        return CryptoKeysConfig(  # type: ignore  # 2024-01-22 # TODO: Unexpected keyword argument "actual_key_id" for "CryptoKeysConfig"  [call-arg]
...
def get_single_key_crypto_keys_config(*, key_id: str, key_value: str) -> CryptoKeysConfig:
    return CryptoKeysConfig(  # type: ignore  # 2024-01-22 # TODO: Unexpected keyword argument "map_id_key" for "CryptoKeysConfig"  [call-arg]
...
class _CryptoKeysConfigOnlySettings(SettingsBase):
    ...
    cry: CryptoKeysConfig = s_attrib("DL_CRY", json_converter=CryptoKeysConfig.from_json, sensitive=True)  # type: ignore  # 2024-01-22 # TODO: Incompatible types in assignment (expression has type "Attribute[Any]", variable has type "CryptoKeysConfig")  [assignment]
