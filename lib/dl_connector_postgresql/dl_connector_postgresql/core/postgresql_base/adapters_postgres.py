...
class PostgresAdapter(BasePostgresAdapter, BaseClassicAdapter[PostgresConnTargetDTO]):
    ...
    def _get_tables(self, schema_ident: SchemaIdent) -> List[TableIdent]:
        ...
        if schema_ident.schema_name is not None:
            ...
            result = db_engine.execute(sa.text(self._LIST_ALL_TABLES_QUERY))  # type: ignore  # 2024-01-22 # TODO: Incompatible types in assignment (expression has type "CursorResult", variable has type "Generator[tuple[str, Any], None, None]")  [assignment]
