...
DEFINITIONS_ARRAY = [
    ...
    base.FuncGetArrayItemFloat(
        variants=[
            V(
                ...
                lambda array, index: Grouping(sa.type_coerce(array, sa_postgresql.ARRAY(TypeEngine)))[index],  # type: ignore  # 2024-01-22 # TODO: Argument 2 has incompatible type "Callable[[Any, Any], ColumnOperators[Any]]"; expected "Callable[..., ClauseElement | FormulaItem]"  [arg-type]
            ),
        ]
    ),
    ...
    base.FuncArraySlice(
        variants=[
            V(
                D.POSTGRESQL,
                lambda array, offset, length: Grouping(sa.type_coerce(array, sa_postgresql.ARRAY(TypeEngine)))[  # type: ignore  # 2024-01-22 # TODO: Argument 2 has incompatible type "Callable[[Any, Any, Any], ColumnOperators[Any]]"; expected "Callable[..., ClauseElement | FormulaItem]"  [arg-type]
    ...
    base.FuncStartswithArrayConst(
        variants=[
            V(
                D.POSTGRESQL,
                lambda x, y, _env: Grouping(sa.type_coerce(x, sa_postgresql.ARRAY(TypeEngine)))[1 : len(un_literal(y))]  # type: ignore  # 2024-01-22 # TODO: Invalid index type "slice" for "Grouping"; expected type "int"  [index]
                == y,
            ),
        ]
    ),
    base.FuncStartswithArrayNonConst(
        variants=[
            V(
                D.POSTGRESQL,
                lambda x, y, _env: Grouping(sa.type_coerce(x, sa_postgresql.ARRAY(TypeEngine)))[  # type: ignore  # 2024-01-22 # TODO: Invalid index type "slice" for "Grouping"; expected type "int"  [index]
