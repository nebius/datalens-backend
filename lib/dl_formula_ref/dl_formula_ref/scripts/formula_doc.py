...
class FormulaDocTool:
    @staticmethod
    def print_versions():  # type: ignore  # 2024-01-22 # TODO: Function is missing a return type annotation  [no-untyped-def]
        print("\n".join(sorted([version.name for version in ConfigVersion])))

    @staticmethod
    def print_locales(config_version: ConfigVersion):  # type: ignore  # 2024-01-22 # TODO: Function is missing a return type annotation  [no-untyped-def]
        gen_config = get_generator_config(version=config_version)
        print("\n".join(sorted(gen_config.supported_locales)))

    @classmethod
    def generate_doc(cls, outdir: str, locale: str, config_version: ConfigVersion):  # type: ignore  # 2024-01-22 # TODO: Function is missing a return type annotation  [no-untyped-def]
