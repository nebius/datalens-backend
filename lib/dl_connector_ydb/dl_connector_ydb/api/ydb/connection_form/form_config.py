...
class YDBConnectionFormFactory(ConnectionFormFactory):
    def _get_base_common_api_schema_items(self, names_source: Type[Enum]) -> list[FormFieldApiSchema]:
        return [
            FormFieldApiSchema(name=names_source.host, required=True),  # type: ignore  # 2024-01-22 # TODO: "type[Enum]" has no attribute "host"  [attr-defined]
            FormFieldApiSchema(name=names_source.port, required=True),  # type: ignore  # 2024-01-22 # TODO: "type[Enum]" has no attribute "port"  [attr-defined]
            FormFieldApiSchema(name=names_source.db_name, required=True),  # type: ignore  # 2024-01-22 # TODO: "type[Enum]" has no attribute "db_name"  [attr-defined]
        ]
    ...
    def get_form_config(
        ...
        common_api_schema_items.append(
            FormFieldApiSchema(
                name=CommonFieldName.token,
                required=self.mode == ConnectionFormMode.create and connector_settings.HAS_AUTH,  # type: ignore  # 2024-01-22 # TODO: Argument "required" to "FormFieldApiSchema" has incompatible type "bool | None"; expected "bool"  [arg-type]
            )
        )
