...
def _order_by_from_args(*args: ClauseElement) -> ClauseList:
    ...
    if len(args) == 2:
        ...
        dir_value = args[1].value.lower()  # type: ignore  # 2024-01-22 # TODO: "ClauseElement" has no attribute "value"  [attr-defined]
    else:
...
def _rows_stretching_window(_: ClauseElement, direction: ClauseElement = sa.literal("asc")) -> WinRangeTuple:
    ...
    assert isinstance(un_literal(direction), str)  # type: ignore  # 2024-01-22 # TODO: Argument 1 to "un_literal" has incompatible type "ClauseElement"; expected "BaseLiteral | BindParameter[Any] | TypeDefiningCast | Function | Null | array | None"  [arg-type]
    dir_value = direction.value.lower()  # type: ignore  # 2024-01-22 # TODO: "ClauseElement" has no attribute "value"  [attr-defined]
    if dir_value == "asc":
        return None, 0
    elif dir_value == "desc":
        return 0, None
