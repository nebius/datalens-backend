...
class FuncDatetimeTZ(TypeConvFunction):
    ...
    return_type_params = ParamsCustom(
        lambda args: DataTypeParams(timezone=un_literal(args[1].expression))  # type: ignore  # 2024-02-01 # TODO: Argument 1 to "un_literal" has incompatible type "ClauseElement | None"; expected "BaseLiteral | BindParameter[Any] | TypeDefiningCast | Function | Null | array | None"  [arg-type]
    )
...
class FuncDatetimeTZConst(SingleVariantTranslationBase, FuncDatetimeTZ):
    ...
    @classmethod
    def _translate_main(cls, value_ctx, tz_ctx, *, _env: TranslationEnvironment):  # type: ignore  # 2024-02-01 # TODO: Function is missing a return type annotation  [no-untyped-def]
...
class FuncDbCastBase(TypeConvFunction):
    ...
    def generate_cast_type(
        ...
        name = un_literal(wr_name.expression)  # type: ignore  # 2024-02-01 # TODO: Argument 1 to "un_literal" has incompatible type "ClauseElement | None"; expected "BaseLiteral | BindParameter[Any] | TypeDefiningCast | Function | Null | array | None"  [arg-type]
        assert isinstance(name, str)
        name = name.upper()
        data_type = value.data_type.non_const_type  # type: ignore  # 2024-02-01 # TODO: Item "None" of "DataType | None" has no attribute "non_const_type"  [union-attr]
        ...
        for arg_num, (arg, req_arg_type) in enumerate(zip(type_args, type_spec.arg_types)):
            if isinstance(req_arg_type, DataType):
                if not arg.data_type.casts_to(req_arg_type):  # type: ignore  # 2024-02-01 # TODO: Item "None" of "DataType | None" has no attribute "casts_to"  [union-attr]
                    _raise_type_mismath(arg_num)
            elif isinstance(req_arg_type, tuple):
                for sub_req_arg_type in req_arg_type:
                    if not arg.data_type.casts_to(sub_req_arg_type):  # type: ignore  # 2024-02-01 # TODO: Item "None" of "DataType | None" has no attribute "casts_to"  [union-attr]
                        _raise_type_mismath(arg_num)
            else:
                raise TypeError(type(req_arg_type))

            unwrapped_native_type_args.append(un_literal(arg.expression))  # type: ignore  # 2024-02-01 # TODO: Argument 1 to "un_literal" has incompatible type "ClauseElement | None"; expected "BaseLiteral | BindParameter[Any] | TypeDefiningCast | Function | Null | array | None"  [arg-type]
