...
def _normalize_arr_items(val: List[TranslationCtx], item_type: Type) -> list:
    ...
    result = []
    for item in val:
        un_lit_item = un_literal(item.expression)  # type: ignore  # 2024-01-22 # TODO: Argument 1 to "un_literal" has incompatible type "ClauseElement | None"; expected "BaseLiteral | BindParameter[Any] | TypeDefiningCast | Function | Null | array | None"  [arg-type]
