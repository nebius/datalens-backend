...
class DefaultColumnRenderer(ColumnRenderer):
    def make_column(self, name: str) -> sa.sql.ClauseElement:
        ...
        return sa.literal_column(".".join([self._quoter(part) for part in full_name_parts]))  # type: ignore  # 2024-01-22 # TODO: List comprehension has incompatible type List[ClauseElement]; expected List[str]  [misc]
