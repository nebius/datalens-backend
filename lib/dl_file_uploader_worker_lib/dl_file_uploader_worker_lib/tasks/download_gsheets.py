...
async def _get_gsheets_auth(
    ...
    if dfile_token is not None:  # if there is a token in dfile, then use it
        ...
        conn: GSheetsFileS3Connection = await usm.get_by_id(conn_id, GSheetsFileS3Connection)  # type: ignore  # 2024-01-22 # TODO: Incompatible types in assignment (expression has type "USEntry", variable has type "GSheetsFileS3Connection")  [assignment]
