...
async def _get_yadocs_oauth_token(
    ...
    if dfile_token is not None:  # if there is a token in dfile, then use it
        ...
        conn: YaDocsFileS3Connection = await usm.get_by_id(conn_id, YaDocsFileS3Connection)  # type: ignore  # 2024-01-22 # TODO: Incompatible types in assignment (expression has type "USEntry", variable has type "YaDocsFileS3Connection")  [assignment]
