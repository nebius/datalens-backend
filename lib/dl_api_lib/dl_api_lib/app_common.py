...
class SRFactoryBuilder(Generic[TSettings], abc.ABC):
    ...
    def get_sr_factory(
        ...
        sr_factory = DefaultApiSRFactory(
            ...
            default_cache_ttl_config=self._get_default_cache_ttl_settings(settings),  # type: ignore  # 2024-01-22 # TODO: Argument "default_cache_ttl_config" to "DefaultApiSRFactory" has incompatible type "CacheTTLConfig | None"; expected "CacheTTLConfig"  [arg-type]
            ...
            entity_usage_checker=self._get_entity_usage_checker(settings),  # type: ignore  # 2024-01-22 # TODO: Argument "entity_usage_checker" to "DefaultApiSRFactory" has incompatible type "EntityUsageChecker | None"; expected "EntityUsageChecker"  [arg-type]
            field_id_generator_type=settings.FIELD_ID_GENERATOR_TYPE,
            file_uploader_settings=file_uploader_settings,  # type: ignore  # 2024-01-22 # TODO: Argument "file_uploader_settings" to "DefaultApiSRFactory" has incompatible type "FileUploaderSettings | None"; expected "FileUploaderSettings"  [arg-type]
            redis_pool_settings=create_arq_redis_settings(settings.REDIS_ARQ) if settings.REDIS_ARQ else None,  # type: ignore  # 2024-01-22 # TODO: Argument "redis_pool_settings" to "DefaultApiSRFactory" has incompatible type "RedisSettings | None"; expected "RedisSettings"  [arg-type]
            rqe_caches_settings=self._get_rqe_caches_settings(settings),  # type: ignore  # 2024-01-22 # TODO: Argument "rqe_caches_settings" to "DefaultApiSRFactory" has incompatible type "RQECachesSetting | None"; expected "RQECachesSetting"  [arg-type]
            ...
            inst_specific_sr_factory=self._get_inst_specific_sr_factory(settings),  # type: ignore  # 2024-01-22 # TODO: Argument "inst_specific_sr_factory" to "DefaultApiSRFactory" has incompatible type "InstallationSpecificServiceRegistryFactory | None"; expected "InstallationSpecificServiceRegistryFactory"  [arg-type]
