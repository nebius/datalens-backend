    ...
    def from_settings(cls, settings: SectionSettings) -> Section:
        def _connector_from_settings(settings: ConnectorBaseSettings | ObjectLikeConfig) -> ConnectorBase:
            if hasattr(settings, "conn_type"):
                ...
                return ConnectorContainer.from_settings(settings)   # type: ignore  # 2024-01-22 # TODO: Argument 1 to "from_settings" of "ConnectorContainer" has incompatible type "ConnectorBaseSettings"; expected "ConnectorContainerSettings"  [arg-type]
...
@attr.s(kw_only=True)
class Connector(ConnectorBase):  # type: ignore  # 2024-01-22 # TODO: Function is missing a type annotation for one or more arguments  [no-untyped-def]
    """Represents an actual connection type"""

    _hidden: bool = attr.ib(init=False, default=False)
    conn_type: ConnectionType = attr.ib(  # type: ignore  # 2024-01-22 # TODO: Need type annotation for "conn_type"  [var-annotated]
...
class ConnectorAvailabilityConfig(SettingsBase):
    ...
    def from_settings(
        ...
        return cls(  # type: ignore  # 2024-01-22 # TODO: Unexpected keyword argument "visible_connectors" for "ConnectorAvailabilityConfig"  [call-arg]
