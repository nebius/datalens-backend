...
class DatasetResource(BIResource):
    ...
    def get_dataset(cls, dataset_id: Optional[str], body: dict) -> Tuple[Dataset, DatasetUpdateInfo]:
        ...
        update_info = loader.update_dataset_from_body(
            dataset=dataset,  # type: ignore  # 2024-01-22 # TODO: Argument "dataset" to "update_dataset_from_body" of "DatasetApiLoader" has incompatible type "USEntry"; expected "Dataset"  [arg-type]
            us_manager=us_manager,
            dataset_data=body.get("dataset"),
        )
        return dataset, update_info  # type: ignore  # 2024-01-22 # TODO: Incompatible return value type (got "tuple[USEntry, DatasetUpdateInfo]", expected "tuple[Dataset, DatasetUpdateInfo]")  [return-value]
