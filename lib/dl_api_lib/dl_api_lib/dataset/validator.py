...
class DatasetValidator(DatasetBaseWrapper):
    ...
    def _migrate_source_parameters(
        ...
        try:
            new_dsrc_spec = dst_migrator.import_migration_dtos(
                ...
                connection_ref=new_connection.conn_ref,  # type: ignore  # 2024-01-22 # TODO: Argument "connection_ref" to "import_migration_dtos" of "DataSourceMigrator" has incompatible type "ConnectionRef | None"; expected "ConnectionRef"  [arg-type]
            )
        except common_exc.DataSourceMigrationImpossible:
            # Failed to migrate anything.
            # Let's just go with an empty default data source
            return {}, new_connection.source_type  # type: ignore  # 2024-01-22 # TODO: Incompatible return value type (got "tuple[dict[Any, Any], DataSourceType | None]", expected "tuple[dict[Any, Any], DataSourceType]")  [return-value]
    ...
    def apply_connection_action(
        ...
        for dsrc_data in updates:
            LOGGER.info(
                f'Going to update source {dsrc_data["id"]} as {dsrc_data["source_type"].name} '  # type: ignore  # 2024-01-22 # TODO: "object" has no attribute "name"  [attr-defined]
