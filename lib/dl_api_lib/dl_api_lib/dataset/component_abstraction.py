...
class DatasetComponentAbstraction:
    ...
    def validate_component_can_be_managed(self, component_ref: DatasetComponentRef, by: Optional[ManagedBy]) -> None:
        if by is not None:
            component = self.get_component(component_ref=component_ref)
            if component.managed_by != by:  # type: ignore  # 2024-02-01 # TODO: Item "ResultSchema" of "DataSourceCollectionBase | SourceAvatar | AvatarRelation | BIField | ObligatoryFilter | ResultSchema | None" has no attribute "managed_by"  [union-attr]
