    ...
    def execute_by_steps(self, db_adapter_query: DBAdapterQuery) -> Generator[ExecutionStep, None, None]:
        ...
        with db_session_context(
            ...
            cursor_info = ExecutionStepCursorInfo(
                cursor_info=self._make_cursor_info(result.cursor, db_session=db_session),  # type: ignore  # 2024-01-22 # TODO: "Result" has no attribute "cursor"  [attr-defined]
                raw_cursor_description=list(result.cursor.description),  # type: ignore  # 2024-01-22 # TODO: "Result" has no attribute "cursor"  [attr-defined]
                raw_engine=engine,
            )
            yield cursor_info
