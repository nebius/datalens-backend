...
class CalcOpExecutorAsync(OpExecutorAsync):
    ...
    def make_data_key(self, op: BaseOp) -> LocalKeyRepresentation:
        ...
        # Legacy procedure
        from_info = self.get_from_info_from_stream(source_stream=source_stream)  # type: ignore  # 2024-01-22 # TODO: Argument "source_stream" to "get_from_info_from_stream" of "CalcOpExecutorAsync" has incompatible type "AbstractStream | None"; expected "AbstractStream"  [arg-type]
        ...
        legacy_data_key = self.db_ex_adapter.get_data_key(
            query=query,
            user_types=source_stream.user_types,  # type: ignore  # 2024-01-22 # TODO: Item "None" of "AbstractStream | None" has no attribute "user_types"  [union-attr]
            from_info=from_info,
            base_key=source_stream.data_key,  # type: ignore  # 2024-01-22 # TODO: Item "None" of "AbstractStream | None" has no attribute "data_key"  [union-attr]
        )

        # New procedure
        new_data_key = source_stream.data_key.extend("query", op.data_key_data)  # type: ignore  # 2024-01-22 # TODO: Item "None" of "AbstractStream | None" has no attribute "data_key"  [union-attr]

        LOGGER.info(
            f"Preliminary cache key info for query: "  # type: ignore  # 2024-01-22 # TODO: Item "None" of "LocalKeyRepresentation | None" has no attribute "key_parts_hash"  [union-attr]
    ...
    async def execute(self, op: BaseOp) -> DataSourceVS:  # type: ignore  # TODO: fix
        ...
        return DataSourceVS(
            ...
            user_types=user_types,
            data_key=data_key,
            meta=DataRequestMetaInfo(data_source_list=prep_src_info.data_source_list),  # type: ignore  # 2024-01-22 # TODO: Argument "data_source_list" to "DataRequestMetaInfo" has incompatible type "tuple[DataSource, ...] | None"; expected "Collection[DataSource]"  [arg-type]
            preparation_callback=source_stream.prepare,
        )
...
class JoinOpExecutorAsync(OpExecutorAsync):
    ...
    async def execute(self, op: BaseOp) -> JointDataSourceVS:  # type: ignore  # TODO: fix
        ...
        return JointDataSourceVS(
            ...
            meta=DataRequestMetaInfo(
                data_source_list=joint_dsrc_info.data_source_list,  # type: ignore  # 2024-01-22 # TODO: Argument "data_source_list" to "DataRequestMetaInfo" has incompatible type "tuple[DataSource, ...] | None"; expected "Collection[DataSource]"  [arg-type]
            ),
            data_key=data_key,
...
class UploadOpExecutorAsync(OpExecutorAsync):
    ...
    def make_data_key(self, op: BaseOp) -> LocalKeyRepresentation:
        ...
        source_stream = self.ctx.get_stream(op.source_stream_id)
        return source_stream.data_key  # type: ignore  # 2024-01-22 # TODO: Item "None" of "AbstractStream | None" has no attribute "data_key"  [union-attr]
