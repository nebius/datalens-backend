...
def raise_for_status_and_hide_secret_headers(response: ClientResponse) -> None:
    ...
    if not response.ok:
        ...
        new_request_info = RequestInfo(
            ...
            CIMultiDict(clean_secret_data_in_headers(_multidict_to_list(response.request_info.headers))),  # type: ignore  # 2024-01-22 # TODO: Argument 3 to "RequestInfo" has incompatible type "CIMultiDict[str]"; expected "CIMultiDictProxy[str]"  [arg-type]
        )
