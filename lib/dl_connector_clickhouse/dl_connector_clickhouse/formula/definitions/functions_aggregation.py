...
def _top_concat_ch(expr: ClauseElement, amount: ClauseElement, sep: str = ", ") -> ClauseElement:
    ...
    res = expr
    # Any --agg-> Array(Any)
    res = chsq_clauses.ParametrizedFunction("topK", [amount], expr)
...
DEFINITIONS_AGG = [
    ...
    base.AggAvgIf(
        variants=[
            V(
                D.CLICKHOUSE,
                lambda expr, cond: getattr(sa.func, "if")(
    ...
    base.AggQuantile(
        variants=[
            V(
                D.CLICKHOUSE,
                lambda expr, quant: chsq_clauses.ParametrizedFunction(
                    "quantileExact", [literal(quantile_value(un_literal(quant)), d=D.CLICKHOUSE)], expr
                ),
            ),
        ]
    ),
    ...
    base.AggQuantileApproximate(
        variants=[
            V(
                D.CLICKHOUSE,
                lambda expr, quant: chsq_clauses.ParametrizedFunction(
                    "quantile", [literal(quantile_value(un_literal(quant)), d=D.CLICKHOUSE)], expr
                ),
            ),
        ]
    ),
