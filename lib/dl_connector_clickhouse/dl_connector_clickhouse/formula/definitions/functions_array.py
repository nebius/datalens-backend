...
DEFINITIONS_ARRAY = [
    ...
    base.FuncArrayRemoveLiteralNull(
        variants=[
            V(
                D.CLICKHOUSE,
                lambda array, value: sa.func.arrayMap(
    ...
    base.FuncArrayRemoveDefault(
        variants=[
            V(
                D.CLICKHOUSE,
                lambda array, value: sa.func.arrayFilter(Lambda(lambda x: sa.func.ifNull(x != value, 1)), array),
            ),
        ]
    ),
    # arr_str
        ...
        variants=[
            V(
                ...
                lambda array, sep, null_str: sa.func.arrayStringConcat(
                    sa.func.arrayMap(
                        Lambda(lambda x: sa.cast(sa.func.ifNull(sa.func.toString(x), null_str), ch_types.String)), array
                    ),
                    sep,
                ),
            ),
    ...
    base.FuncIntArrayFromFloatArray(
        variants=[
            V(D.CLICKHOUSE, lambda array: sa.func.arrayMap(Lambda(lambda x: sa.func.toInt64(sa.func.floor(x))), array)),
        ]
    ),
    ...
    base.FuncStringArrayFromIntArray(
        variants=[
            V(D.CLICKHOUSE, lambda array: sa.func.arrayMap(Lambda(lambda x: sa.func.toString(x)), array)),
        ]
    ),
    ...
    base.FuncArrayContainsSubsequence(
        variants=[
    ...
    base.FuncReplaceArrayLiteralNull(
        variants=[
            V(D.CLICKHOUSE, lambda array, old, new: sa.func.arrayMap(Lambda(lambda x: sa.func.ifNull(x, new)), array)),
        ]
    ),
    base.FuncReplaceArrayDefault(
        variants=[
            V(
                D.CLICKHOUSE,
                lambda array, old, new: sa.func.arrayMap(Lambda(lambda x: sa.func.if_(x == old, new, x)), array),
            ),
        ]
    ),
    # slice
